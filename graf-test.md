...
graph TD
    ...
    User -- "has role" --> Mentor
    User --> Resume
    User --> ProfilePermissions
    User --> NotificationUserSettings

    Course -- "contains" --> Student
    Course -- "contains" --> Mentor
    Course -- "schedules" --> CourseTask
    Course -- "schedules" --> CourseEvent
    Course -- "accepts" --> Registry
    Course -- "organizes" --> TeamDistribution
    
    Task -- "is instantiated as" --> CourseTask

    CourseTask -- "produces" --> TaskResult
    CourseTask -- "produces" --> TaskSolution
    CourseTask -- "produces" --> TaskVerification
    CourseTask -- "leads to" --> TaskInterviewStudent

    TaskSolution -- "is reviewed via" --> TaskSolutionChecker
    TaskSolutionChecker -- "results in" --> TaskSolutionResult

    TaskInterviewStudent -- "results in" --> TaskInterviewResult

    TeamDistribution -- "forms" --> Team
    TeamDistribution -- "tracks" --> TeamDistributionStudent
